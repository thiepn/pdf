...
const globalCommands: Array<{ label: string; description: string; route: AppRoute; keywords: string }> = [
  ...
  { label: "PDF tools", description: "Find merge, scan, batch, compare, create, convert, and document tools", route: { name: "tools" }, keywords: "merge scan batch compare create convert crop split watermark metadata" },
  { label: "Documents", description: "Manage local projects and backups", route: { name: "projects" }, keywords: "projects backup restore local" },
  { label: "Help", description: "Search workflows, limitations, and shortcuts", route: { name: "help" }, keywords: "guide documentation shortcuts help" },
  { label: "Troubleshooting & recovery", description: "Recover projects or repair the application when something goes wrong", route: { name: "maintenance" }, keywords: "safe mode repair recovery cache support" },
  { label: "Settings", description: "Appearance, recovery, workspace, and privacy controls", route: { name: "settings" }, keywords: "theme privacy updates performance accessibility" }
];

// R1 deliberately excludes engineering-only Inspect/Preservation and recovery-only
// Repair from normal command search. Their routes remain available from Help/About.
// R2 replaces this mode list with canonical task actions and synonyms.
const workspaceModes: WorkspaceMode[] = ["viewer", "editor", "organizer", "toolbox", "compress", "secure", "ocr", "compliance", "professional"];
...
export function CommandPalette() {
  ...
  return <div className="command-palette-backdrop" onMouseDown={(event: MouseEvent<HTMLDivElement>) => { if (event.currentTarget === event.target) closePalette(); }} role="presentation">
    <section aria-describedby="command-palette-help" aria-labelledby="command-palette-title" aria-modal="true" className="command-palette" ref={dialogRef} role="dialog">
      ...
      <input aria-controls="command-palette-results" aria-label="Search commands" onChange={(event: ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} placeholder="Search PDF tools, documents, settings, or help…" ref={inputRef} value={query}/>
