...
function nativeWorker(): Worker { return new Worker(new URL("../workers/native-editor.worker.ts", import.meta.url), { type: "module" }); }
function imageWorker(): Worker { return new Worker(new URL("../workers/native-image.worker.ts", import.meta.url), { type: "module" }); }
...
function mergeVectorInspection(base: NativeInspection, vector: VectorInspection): NativeInspection {
  const pages = base.pages.map((page) => {
    ...
    const insertion = firstLegacyVector >= 0 ? Math.min(firstLegacyVector, withoutLegacy.length) : Math.min(firstTableOrForm < 0 ? withoutLegacy.length : firstTableOrForm, withoutLegacy.length);
    return { ...page, objects: [...withoutLegacy.slice(0, insertion), ...replacement, ...withoutLegacy.slice(insertion)] };
  });
  return registerNativeInspectionPages({ ...base, pages, totals: { ...base.totals, vectors: vector.total }, warnings: [...base.warnings.filter((warning) => !/vector/i.test(warning)), ...vector.warnings] });
}
...
export async function inspectNativePdf(bytes: Uint8Array, password?: string, signal?: AbortSignal): Promise<NativeInspection> {
  ...
  const timeout = globalThis.setTimeout(() => { timedOut = true; controller.abort(); }, NATIVE_INSPECTION_TIMEOUT_MS);
  try {
...
export async function applyNativeEdits(bytes: Uint8Array, edits: NativeEdit[], password?: string, signal?: AbortSignal) {
  ...
  if (otherEdits.length) {
    ...
    working = result.bytes; nativeReport = result.report;
  }
  if (vectorEdits.length) {
    ...
    working = result.bytes; vectorReport = result.report;
  }
  if (tableEdits.length) {
    ...
    working = result.bytes; tableReport = result.report;
  }
  if (imageEdits.length) {
    ...
    working = result.bytes; imageReport = result.report;
  }
  if (complexEdits.length) {
    ...
    working = result.bytes; complexReport = result.report;
  }
