...
export function ViewerPage({ projectId, onTitleChange, readOnly = false }: ViewerPageProps) {
  ...
  const openDocument = useCallback(async (manifest: ProjectManifest, bytes: Uint8Array, suppliedPassword?: string) => {
    ...
    try {
      ...
      const [labelsResult, outlineResult, metadataResult, savedPreferences] = await Promise.all([
        ...
        readViewerPreferences(manifest.id)
      ]);
      ...
      if (/password|encrypted/i.test(message)) {
        ...
        setError("This local project is encrypted. Enter the PDF password to open it for this session.");
      } else {
        setError(message);
      }
    } finally {
  ...
  useEffect(() => {
    if (project) onTitleChange?.(project.name, `${project.summary.pageCount} pages · ${formatBytes(project.byteLength)}`);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      void writeViewerPreferences({ ...preferences, updatedAt: Date.now() });
    }, 350);
    return () => {
      if (saveTimerRef.current) window.clearTimeout(saveTimerRef.current);
    };
  ...
  const changePreferences = useCallback((patch: Partial<ViewerPreferences>) => {
  ...
  const jumpToPage = useCallback((pageNumber: number) => {
    ...
    if (preferences.viewMode === "continuous") {
      window.requestAnimationFrame(() => {
        window.document.querySelector(`[data-page-number="${bounded}"]`)?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }
  }, [changePreferences, pdfDocument, preferences.viewMode]);

  const handlePageVisible = useCallback((pageNumber: number) => {
    changePreferences({ pageNumber });
  }, [changePreferences]);
  ...
  async function runSearch(): Promise<void> {
    ...
    setSearching(true);
    setSearchResults([]);
    setSearchProgress({ done: 0, total: pdfDocument.numPages });
    changePreferences({ sidebarOpen: true, sidebarTab: "search" });
    try {
      const results = await searchPdfDocument(
        pdfDocument,
        searchQuery,
        { caseSensitive: searchCase, wholeWord: searchWhole },
        controller.signal,
  ...
  async function retryPassword(): Promise<void> {
    if (!project || !bytesRef.current || !password) return;
    await openDocument(project, bytesRef.current, password);
  }
  ...
  return (
    <div className="viewer-app">
      <div className="viewer-commandbar">
        ...
        <div className="viewer-commandbar__actions">
          {performancePolicy.largeDocument ? <span className="viewer-performance-badge" title={performancePolicy.reasons.join(" ")}>{performancePolicy.extremeDocument ? "Very large" : "Large"} document · {performancePolicy.effectiveProfile}</span> : null}{editorObjectCount ? <span className="viewer-edit-count">{editorObjectCount} local edit{editorObjectCount === 1 ? "" : "s"}</span> : null}
          <button className="button button--ghost button--small" onClick={downloadOriginal} type="button">Download original PDF</button>
          <button className="button button--small" onClick={() => void backupProject()} type="button">Back up project</button>
        </div>
      </div>
      ...
      {passwordRequired ? (
        <div className="viewer-password-overlay" role="presentation">
          <div aria-describedby="viewer-password-description" aria-labelledby="viewer-password-title" aria-modal="true" className="viewer-password-dialog" ref={passwordDialogRef} role="dialog">
            <p className="eyebrow">Protected document</p><h2 id="viewer-password-title">Password required</h2>
            <p id="viewer-password-description">The password is kept in memory only for this viewing session.</p>
            <label className="visually-hidden" htmlFor="viewer-password-input">PDF password</label>
            <input autoComplete="off" id="viewer-password-input" onChange={(event: { target: HTMLInputElement }) => setPassword(event.target.value)} placeholder="PDF password" ref={passwordInputRef} type="password" value={password} />
            <div className="button-row"><button className="button" disabled={!password} onClick={() => void retryPassword()} type="button">Open locally</button><button className="button button--ghost" onClick={closePasswordDialog} type="button">Cancel</button></div>
          </div>
        </div>
      ) : null}
      ...
      <div className={preferences.sidebarOpen ? "viewer-layout" : "viewer-layout viewer-layout--collapsed"}>
        <aside aria-label="Document navigation" className="viewer-sidebar" id="viewer-sidebar">
          <div aria-label="Viewer side panel" className="viewer-sidebar__tabs" role="tablist">
            {(["pages", "outline", "search", "info"] as const).map((tab) => <button aria-controls="viewer-sidebar-panel" aria-selected={preferences.sidebarTab === tab} className={preferences.sidebarTab === tab ? "active" : ""} id={`viewer-tab-${tab}`} key={tab} onClick={() => changePreferences({ sidebarTab: tab })} role="tab" tabIndex={preferences.sidebarTab === tab ? 0 : -1} type="button">{tab}</button>)}
          </div>
          <div aria-labelledby={`viewer-tab-${preferences.sidebarTab}`} className="viewer-sidebar__body" id="viewer-sidebar-panel" role="tabpanel">
            {preferences.sidebarTab === "pages" && pdfDocument ? (
              <div className="thumbnail-list">{Array.from({ length: pdfDocument.numPages }, (_, index) => <Thumbnail document={pdfDocument} key={index + 1} label={pageLabels?.[index]} onSelect={jumpToPage} pageNumber={index + 1} scheduler={renderScheduler} selected={preferences.pageNumber === index + 1} />)}</div>
            ) : null}
            {preferences.sidebarTab === "outline" ? <OutlineTree nodes={outline} onSelect={(node) => void resolveOutlineDestination(node)} /> : null}
            {preferences.sidebarTab === "search" ? (
              <SearchSidebar
                caseSensitive={searchCase}
                onCaseChange={setSearchCase}
                onCancel={() => searchAbortRef.current?.abort()}
                onQueryChange={setSearchQuery}
                onRun={() => void runSearch()}
                onSelect={jumpToPage}
                onWholeChange={setSearchWhole}
        ...
        <main aria-label="PDF document pages" className="document-stage">
          {!pdfDocument ? <div aria-live="polite" className="viewer-loading" role="status"><span aria-hidden="true" className="spinner" /><strong>Opening PDF…</strong></div> : preferences.viewMode === "single" ? (
            <div className="single-page-stage"><PageCanvas document={pdfDocument} pageNumber={preferences.pageNumber} pixelRatioCap={performancePolicy.pixelRatioCap} scheduler={renderScheduler} searchQuery={searchQuery} zoom={preferences.zoom} /></div>
          ) : (
            <div className="continuous-page-stage">
              {Array.from({ length: pdfDocument.numPages }, (_, index) => (
                <PageCanvas
                  ...
                  pageNumber={index + 1}
                  pixelRatioCap={performancePolicy.pixelRatioCap}
                  scheduler={renderScheduler}
                  activationMarginPx={performancePolicy.activationMarginPx}
                  evictionDistanceScreens={performancePolicy.evictionDistanceScreens}
...
function normalizePreferences(preferences: ViewerPreferences, pageCount: number): ViewerPreferences {
  return {
    ...preferences,
    pageNumber: Math.max(1, Math.min(pageCount, preferences.pageNumber || 1)),
    zoom: Math.max(0.25, Math.min(4, preferences.zoom || 1)),
    viewMode: preferences.viewMode === "single" ? "single" : "continuous",
    sidebarTab: ["pages", "outline", "search", "info"].includes(preferences.sidebarTab) ? preferences.sidebarTab : "pages",
...
interface SearchSidebarProps {
  ...
  onWholeChange: (value: boolean) => void;
  onRun: () => void;
  onCancel: () => void;
...
function SearchSidebar(props: SearchSidebarProps) {
  ...
  return <div className="search-sidebar">
    <form onSubmit={(event: { preventDefault(): void }) => { event.preventDefault(); props.onRun(); }}>
      <input aria-label="Search document" autoFocus onChange={(event: { target: HTMLInputElement }) => props.onQueryChange(event.target.value)} placeholder="Search document" type="search" value={props.query} />
      <button className="button button--small" disabled={!props.query.trim() || props.searching} type="submit">Search</button>
    </form>
    <label><input checked={props.caseSensitive} onChange={(event: { target: HTMLInputElement }) => props.onCaseChange(event.target.checked)} type="checkbox" /> Match case</label>
    <label><input checked={props.wholeWord} onChange={(event: { target: HTMLInputElement }) => props.onWholeChange(event.target.checked)} type="checkbox" /> Whole word</label>
    {props.searching && props.progress ? <div aria-live="polite" className="search-progress" role="status"><progress aria-label="Search progress" max={props.progress.total} value={props.progress.done} /><span>{props.progress.done} / {props.progress.total} pages</span><button onClick={props.onCancel} type="button">Cancel</button></div> : null}
    {!props.searching && props.query ? <p className="search-summary">{totalMatches} {totalMatches === 1 ? "match" : "matches"} on {props.results.length} pages</p> : null}
    <div className="search-results">{props.results.map((result) => <button key={result.id} onClick={() => props.onSelect(result.pageNumber)} type="button"><strong>Page {result.pageLabel ?? result.pageNumber}</strong><span>{result.matchCount} {result.matchCount === 1 ? "match" : "matches"}</span><p>{result.snippet}</p></button>)}</div>
  </div>;
}

function InformationSidebar({ metadata, project }: { metadata: DocumentMetadata; project: ProjectManifest }) {
  const rows = [
    ["Filename", project.sourceFilename], ["Pages", project.summary.pageCount], ["File size", formatBytes(project.byteLength)], ["Storage", project.storageKind.toUpperCase()], ["Checksum", `${project.checksum.slice(0, 16)}…`], ["Title", metadata.Title], ["Author", metadata.Author], ["Creator", metadata.Creator], ["Producer", metadata.Producer], ["PDF version", metadata.PDFFormatVersion], ["Forms", project.summary.formFieldCount ?? 0], ["Attachments", project.summary.attachmentCount ?? 0], ["JavaScript", project.summary.hasJavaScript ? "Detected; execution disabled" : "Not detected"]
  ];
  return <dl className="info-list">{rows.map(([label, value]) => <div key={String(label)}><dt>{String(label)}</dt><dd>{value === undefined || value === "" ? "—" : String(value)}</dd></div>)}</dl>;
}

function ViewerFatalError({ error }: { error: string }) {
  return <div className="fatal-state"><strong>Project could not be opened</strong><p>{error}</p><a className="button" href={routeHref({ name: "home" })}>Return home</a></div>;
}
