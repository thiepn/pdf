...
export function ToolsPage() {
  ...
  const ToolButton = ({ target, icon, title, description, primary = false }: { target: LocalTarget; icon: string; title: string; description: string; primary?: boolean }) => <button className={primary ? "tool-tile tool-tile--primary" : "tool-tile"} disabled={busy} onClick={() => chooseFile(target)} type="button"><span>{icon}</span><div><strong>{title}</strong><p>{description}</p></div><small>{busy ? "Opening…" : "Available"}</small></button>;
  ...
  return <div className="tools-page">
    ...
    <section className="tool-category"><div className="section-heading"><div><p className="eyebrow">Organize and create</p><h2>Create, page, and scan tools</h2></div></div><div className="tool-grid">
      <a className="tool-tile tool-tile--primary" href={routeHref({ name: "create" })}><span>✎</span><div><strong>Create PDF</strong><p>Write Markdown, text, or semantic HTML with metric page controls and export a searchable or visual PDF.</p></div><small>Write or paste content</small></a>
      <a className="tool-tile" href={routeHref({ name: "merge" })}><span>＋</span><div><strong>Merge PDFs</strong><p>Combine complete PDFs in a chosen order.</p></div><small>Available</small></a>
      <ToolButton description="Reorder, rotate, duplicate, delete, reverse, or extract pages." icon="▦" target="organizer" title="Organize, split, and extract"/>
      <a className="tool-tile" href={routeHref({ name: "scan" })}><span>▧</span><div><strong>Scan to PDF</strong><p>Create a PDF from images or camera captures, with optional OCR.</p></div><small>Available</small></a>
    </div></section>
    <section className="tool-category"><div className="section-heading"><div><p className="eyebrow">OCR and optimize</p><h2>Searchability and file size</h2></div></div><div className="tool-grid">
      <ToolButton description="Recognize scanned pages and generate searchable PDF output." icon="Aa" primary target="ocr" title="OCR PDF"/>
      <ToolButton description="Use lossless cleanup or stronger raster compression profiles." icon="⇲" target="compress" title="Optimize PDF"/>
      <a className="tool-tile" href={routeHref({ name: "batch" })}><span>≣</span><div><strong>Batch automation</strong><p>Apply the same saved sequence of actions to several PDFs.</p></div><small>Available</small></a>
    </div></section>
    <section className="tool-category"><div className="section-heading"><div><p className="eyebrow">Everyday toolbox</p><h2>Decorate, crop, metadata, and convert</h2></div></div><div className="tool-grid">
      <ToolButton description="Add watermarks, headers, footers, and page numbers; crop pages; insert blank pages; edit metadata; export text, Markdown, HTML, or page images." icon="⌘" primary target="toolbox" title="PDF utilities"/>
      <ToolButton description="Advanced content replacement, document numbering, print layout/booklets, layers, and archive checks." icon="PRO" target="professional" title="Print & Advanced"/>
    </div></section>
    <section className="tool-category"><div className="section-heading"><div><p className="eyebrow">Forms, protection, and comparison</p><h2>Protect or compare documents</h2></div></div><div className="tool-grid">
      <ToolButton description="Fill forms, permanently apply redactions, remove risky active content, flatten, or add password protection." icon="▣" primary target="secure" title="Forms & Protect"/>
      <details className="tool-advanced-disclosure"><summary>Advanced tools</summary><div className="tool-grid tool-grid--nested">
        <ToolButton description="Inspect fonts, images, forms, actions, signatures, revisions, and other technical PDF structure." icon="⌕" target="inspector" title="Inspect PDF structure"/>
        <ToolButton description="Rewrite a separate clean copy when a PDF is damaged or behaves unexpectedly." icon="↻" target="repair" title="Repair PDF"/>
      </div></details>
      <a className="tool-tile" href={routeHref({ name: "compare" })}><span>◫</span><div><strong>Compare PDFs</strong><p>Hybrid-align text or scanned pages, then compare matched pages visually or by text.</p></div><small>Text and scanned PDFs</small></a>
    </div></section>
