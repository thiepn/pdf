...
const relatedTaskIds = new Set(["merge-pdfs", "compare-pdfs"]);
...
export function DocumentToolsPage({ projectId, onTitleChange }: { projectId: string; onTitleChange?: (title: string, subtitle?: string) => void }) {
  ...
  const visible = useMemo(() => {
    ...
    if (needle) return tasks.filter((task) => taskSearchText(task).includes(needle));
    return tasks.filter((task) => task.audience === "everyday");
  }, [query]);

  const advanced = useMemo(() => pdfTasks.filter((task) => task.target.kind === "workspace" && task.audience === "advanced"), []);
  ...
  const related = pdfTasks.filter((task) => relatedTaskIds.has(task.id) && task.audience === "everyday");
  ...
  return <div className="document-tools-hub">
    <section className="document-tools-hub__hero"><div><p className="eyebrow">Current PDF</p><h2>What do you want to do?</h2><p>Choose an outcome. Everyday tasks are shown first, while advanced and troubleshooting tools remain available without crowding the normal workflow.</p></div><label><span className="visually-hidden">Search current PDF tasks</span><input onChange={(event: ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} placeholder="Search crop, redact, OCR, metadata…" type="search" value={query}/></label></section>
    ...
    {capabilityContext && !searching && advanced.length ? <details className="task-advanced-disclosure"><summary><span><strong>Advanced & specialist tools</strong><small>Less common page, protection, accessibility, print, archive, and conversion workflows.</small></span><span>{advanced.length} tools</span></summary><div className="task-advanced-disclosure__body">{taskCategories.map((category) => {
      const tasks = advanced.filter((task) => task.category === category.id).filter((task) => capabilityFor(task)?.state !== "hidden");
      if (!tasks.length) return null;
      return <section className="document-task-section" key={category.id}><header><div><p className="eyebrow">{category.label}</p><h3>{category.description}</h3></div></header><div className="document-task-grid">{tasks.map((task) => <DocumentTask capability={capabilityFor(task) as TaskCapability} key={task.id} projectId={projectId} task={task}/>)}</div></section>;
    })}</div></details> : null}
    ...
    {capabilityContext && !visible.length && !recovery.length ? <div className="empty-state"><strong>No matching task</strong><p>Try edit, pages, protect, convert, compare, create, repair, or accessibility.</p></div> : null}

    {capabilityContext && !query ? <section className="document-task-section"><header><div><p className="eyebrow">Related workflows</p><h3>Other everyday workflows that use more than this one PDF.</h3></div></header><div className="document-task-grid">{related.map((task) => <DocumentTask capability={capabilityFor(task) ?? { state: "available", label: "Ready" }} key={task.id} projectId={projectId} task={task}/>)}</div></section> : null}
  </div>;
}
