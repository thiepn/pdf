...
const tabs: Array<{ id: SecurityTab; label: string; icon: string }> = [
  { id: "overview", label: "Inspector", icon: "◇" },
...
export function SecurePage({ projectId, onTitleChange }: Props) {
  ...
  const [status, setStatus] = useState("Opening secure workspace…");
  ...
  async function openDocument(manifest: ProjectManifest, bytes: Uint8Array, suppliedPassword?: string): Promise<void> {
    setStatus("Inspecting PDF security…"); setError(null);
    try {
      ...
      onTitleChange?.(`Secure · ${manifest.name}`, `${report.pageCount} pages · ${report.formFields.length} fields · ${report.signatures.length} signature fields · local processing`);
    } catch (reason) {
      const message = reason instanceof Error ? reason.message : String(reason);
      if (/password|encrypted/i.test(message)) { setPasswordRequired(true); setError("Enter the PDF password. It remains only in memory for this local session."); }
      else throw reason;
    }
  }
  ...
  async function exportSecure(saveProject: boolean): Promise<void> {
    ...
    setBusy(true); setError(null); setWarnings([]); setValidation(null); setStatus("Compiling editor and security changes…");
    ...
    try {
      ...
      update({ detail: "Compiling editor and security changes…", progress: 0.05 });
      ...
      setStatus("Applying forms, redactions, sanitization, and protection…");
      update({ detail: "Applying forms, redactions, sanitization, and protection…", progress: 0.45 });
      ...
      setStatus("Running security-critical validation…");
      update({ stage: "validating", detail: "Running security-critical validation…", progress: 0.82 });
      ...
      if (!result.valid) throw new Error(`Security validation failed: ${result.checks.filter((check) => !check.passed).map((check) => check.name).join(", ")}. The output was not released.`);
      ...
      if (saveProject) {
        update({ stage: "committing", detail: "Saving a new protected revision…", progress: 0.94 });
  ...
  return <div className="security-app">
    <header className="security-commandbar">
      <div className="editor-file-group"><a className="icon-button" href={routeHref({ name: "viewer", projectId })}>←</a><div><strong>{project.name}</strong><span>{status} · security-critical changes create a new output</span></div></div>
      ...
      <div className="editor-commandbar__actions"><button className="button button--ghost button--small" disabled={busy} onClick={() => void exportSecure(false)} type="button">Download secured PDF</button><button className="button button--small" disabled={busy} onClick={() => void exportSecure(true)} type="button">Save as project</button>{busy ? <button className="button button--danger-ghost button--small" onClick={() => abortRef.current?.abort()} type="button">Cancel</button> : null}</div>
    </header>

    <div className="security-notices">{error ? <div className="editor-banner error-banner"><strong>Forms & Protect blocked</strong><span>{error}</span><button onClick={() => setError(null)} type="button">Dismiss</button></div> : null}{warnings.length ? <div className="editor-banner warning-banner"><strong>Warnings</strong><span>{warnings.join(" ")}</span><button onClick={() => setWarnings([])} type="button">Dismiss</button></div> : null}</div>
...
function renderPanel(tab: SecurityTab, context: PanelContext) {
  ...
  if (tab === "overview") return <div className="security-panel-body"><PanelTitle title="Security inspector" subtitle="Current document state before export" /><div className="security-fact-grid"><Fact label="Encryption" value={inspection.encryptionDescription} /><Fact label="Authentication" value={inspection.authentication} /><Fact label="Revisions" value={String(inspection.versionCount)} /><Fact label="Forms" value={String(inspection.formFields.length)} /><Fact label="Signatures" value={String(inspection.signatures.length)} /><Fact label="Attachments" value={String(inspection.attachmentCount)} /><Fact label="JavaScript" value={inspection.hasJavaScript ? "Detected" : "None detected"} danger={inspection.hasJavaScript} /><Fact label="Automatic actions" value={inspection.hasOpenAction || inspection.hasAdditionalActions ? "Detected" : "None detected"} danger={inspection.hasOpenAction || inspection.hasAdditionalActions} /></div><section className="security-section"><h3>Permissions</h3><div className="permission-list">{Object.entries(inspection.permissions).map(([key, value]) => <span className={value ? "allowed" : "blocked"} key={key}>{value ? "✓" : "×"} {humanize(key)}</span>)}</div></section><section className="security-section"><h3>Metadata</h3>{Object.keys(inspection.metadata).length ? <dl className="security-metadata">{Object.entries(inspection.metadata).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl> : <p className="property-note">No standard metadata fields were detected.</p>}</section></div>;
  if (tab === "forms") return <FormsPanel {...context} />;
  if (tab === "redaction") return <div className="security-panel-body"><PanelTitle title="Permanent redaction" subtitle="Remove marked page content during secure export" /><div className="security-critical-note"><strong>A black rectangle is not enough.</strong><p>Only Apply redactions removes the underlying text, images, and vector content. The output is then rewritten with garbage collection.</p></div><a className="button button--ghost button--block" href={routeHref({ name: "editor", projectId: context.projectId })}>Open editor to place redaction regions</a><section className="security-section"><label className="property-toggle"><input checked={security.redaction.enabled} onChange={(event) => setSecurity((state) => ({ ...state, redaction: { ...state.redaction, enabled: event.target.checked } }))} type="checkbox" />Apply {context.redactionObjects.length + context.inspection.redactionMarkCount} marked region{context.redactionObjects.length + context.inspection.redactionMarkCount === 1 ? "" : "s"}</label><label className="property-toggle"><input checked={security.redaction.blackBoxes} onChange={(event) => setSecurity((state) => ({ ...state, redaction: { ...state.redaction, blackBoxes: event.target.checked } }))} type="checkbox" />Render black boxes after removal</label><label className="property-toggle"><input checked={security.redaction.removeText} onChange={(event) => setSecurity((state) => ({ ...state, redaction: { ...state.redaction, removeText: event.target.checked } }))} type="checkbox" />Remove intersecting text</label><label className="property-field"><span>Images</span><select onChange={(event) => setSecurity((state) => ({ ...state, redaction: { ...state.redaction, imageMode: event.target.value as typeof state.redaction.imageMode } }))} value={security.redaction.imageMode}><option value="pixels">Remove covered pixels</option><option value="remove">Remove entire touched image</option><option value="unless-invisible">Remove visible touched images</option><option value="none">Do not alter images</option></select></label><label className="property-field"><span>Vector line art</span><select onChange={(event) => setSecurity((state) => ({ ...state, redaction: { ...state.redaction, lineArtMode: event.target.value as typeof state.redaction.lineArtMode } }))} value={security.redaction.lineArtMode}><option value="covered">Remove fully covered objects</option><option value="touched">Remove any touched object</option><option value="none">Do not alter line art</option></select></label></section></div>;
...
function FormsPanel(context: PanelContext) {
  ...
  if (!fields.length) return <div className="security-panel-body"><PanelTitle title="Forms" subtitle="Fill existing AcroForm fields" /><div className="security-empty"><strong>No AcroForm fields detected</strong><p>The current MuPDF browser API reliably edits existing widgets but does not yet provide production-ready creation of arbitrary new field types. Form creation remains deferred rather than simulated.</p></div></div>;
  return <div className="security-panel-body"><PanelTitle title="Form values" subtitle={`${fields.length} existing fields · JavaScript execution disabled`} /><div className="security-form-list">{fields.map((field) => <FormControl field={field} key={field.id} selected={context.selectedField?.id === field.id} value={context.security.formValues[field.id] ?? field.value} onChange={(value) => context.setSecurity((state) => ({ ...state, formValues: { ...state.formValues, [field.id]: value }, currentPage: field.pageNumber }))} onSelect={() => context.setSelectedFieldId(field.id)} />)}</div><button className="button button--ghost button--block" onClick={() => context.setSecurity((state) => ({ ...state, formValues: { ...context.initialFormValues } }))} type="button">Reset pending values</button><p className="property-note">Read-only and cryptographic signature fields are displayed but cannot be changed here.</p></div>;
}
...
function SignaturesPanel(context: PanelContext) {
  return <div className="security-panel-body"><PanelTitle title="Signatures" subtitle="Visual marks and existing cryptographic fields" /><div className="security-critical-note security-critical-note--neutral"><strong>Visual and digital signatures are different.</strong><p>Visual signatures are appearance marks. Cryptographic signatures contain a certificate and can be invalidated by later changes.</p></div><a className="button button--ghost button--block" href={routeHref({ name: "editor", projectId: context.projectId })}>Open editor to place a visual signature</a><section className="security-section"><h3>Visual signatures ({context.visualSignatures.length})</h3>{context.visualSignatures.length ? context.visualSignatures.map((signature) => <div className="security-signature-row" key={signature.id}><span>✒</span><div><strong>{signature.signerName || "Signature"}</strong><small>Page {signature.pageNumber} · appearance only</small></div></div>) : <p className="property-note">No visual signature marks are pending.</p>}</section><section className="security-section"><h3>Cryptographic fields ({context.inspection.signatures.length})</h3>{context.inspection.signatures.length ? context.inspection.signatures.map((signature) => <div className="security-signature-row" key={signature.id}><span>{signature.signed ? "✓" : "○"}</span><div><strong>{signature.name}</strong><small>Page {signature.pageNumber} · {signature.signed ? signature.signatory || "signed" : "unsigned"}</small>{signature.digestStatus ? <em>{signature.digestStatus}</em> : null}</div></div>) : <p className="property-note">No cryptographic signature fields were detected.</p>}<p className="property-note">Certificate-based signing is not enabled because the browser MuPDF API does not expose the required signer workflow reliably. The application does not substitute an image and call it a digital signature.</p></section></div>;
}
...
function SanitizePanel({ security, setSecurity, inspection }: { security: SecurityProjectState; setSecurity: PanelContext["setSecurity"]; inspection: SecurityInspectionReport }) {
  const options: Array<{ key: keyof SecurityProjectState["sanitization"]; label: string; detail: string }> = [
    ...
    { key: "removeJavaScript", label: "Remove JavaScript name trees", detail: inspection.hasJavaScript ? "Active content detected" : "No JavaScript detected" },
    ...
    { key: "removeAttachments", label: "Remove embedded attachments", detail: `${inspection.attachmentCount} catalog attachments detected` },
...
function ValidationPanel({ validation, inspection }: { validation: SecurityValidationReport | null; inspection: SecurityInspectionReport }) {
  return <div className="security-validation-body"><PanelTitle title="Validation" subtitle="Output is withheld when a required check fails" />{validation ? <><div className={`security-validation-summary ${validation.valid ? "passed" : "failed"}`}><strong>{validation.valid ? "Passed" : "Failed"}</strong><span>{validation.checks.filter((check) => check.passed).length}/{validation.checks.length} checks</span></div><div className="security-check-list">{validation.checks.map((check) => <div className={check.passed ? "passed" : "failed"} key={check.name}><span>{check.passed ? "✓" : "×"}</span><div><strong>{check.name}</strong><small>{check.detail}</small></div></div>)}</div></> : <div className="security-empty"><strong>No output validated yet</strong><p>Export runs PDF.js and MuPDF reopening, structure checks, sanitization inspection, encryption checks, and text-extraction checks for marked redactions.</p></div>}<section className="security-section"><h3>Source risk summary</h3><ul className="security-risk-list"><li className={inspection.encrypted ? "warn" : "ok"}>{inspection.encrypted ? "Encrypted source" : "Unencrypted source"}</li><li className={inspection.hasJavaScript || inspection.hasOpenAction ? "warn" : "ok"}>{inspection.hasJavaScript || inspection.hasOpenAction ? "Active content detected" : "No active content detected"}</li><li className={inspection.signatures.some((signature) => signature.signed) ? "warn" : "ok"}>{inspection.signatures.some((signature) => signature.signed) ? "Signed document; changes may invalidate signatures" : "No signed fields detected"}</li><li className={inspection.repaired ? "warn" : "ok"}>{inspection.repaired ? "Document required repair" : "No repair reported"}</li></ul></section></div>;
}
...
function PasswordDialog({ error, password, onChange, onSubmit, projectId }: { error: string | null; password: string; onChange: (value: string) => void; onSubmit: () => void; projectId: string }) {
  ...
  return <div className="viewer-password-overlay" role="presentation"><div aria-describedby="secure-password-description" aria-labelledby="secure-password-title" aria-modal="true" className="viewer-password-dialog" ref={dialogRef} role="dialog"><p className="eyebrow">Protected document</p><h2 id="secure-password-title">Password required</h2><p id="secure-password-description">The password remains in memory and is passed only to local PDF engines.</p><label className="visually-hidden" htmlFor="secure-password-input">PDF password</label><input autoComplete="off" id="secure-password-input" onChange={(event) => onChange(event.target.value)} placeholder="PDF password" ref={inputRef} type="password" value={password} /><button className="button" disabled={!password} onClick={onSubmit} type="button">Open secure workspace</button><a className="button button--ghost" href={routeHref({ name: "viewer", projectId })}>Return to viewer</a>{error ? <span aria-live="assertive" className="selection-help selection-help--error" role="alert">{error}</span> : null}</div></div>;
}
