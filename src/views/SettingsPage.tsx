...
export function SettingsPage() {
  ...
  return <div className="settings-grid">
    ...
    <section className="settings-section"><div><p className="eyebrow">Appearance</p><h2>Interface</h2></div>
      ...
      <label><span><strong>Experience level</strong><small>Simple shows everyday PDF tools. Advanced adds specialist inspection, repair, print, accessibility, and document-structure tools.</small></span><select onChange={(event: { target: HTMLSelectElement }) => update("experienceMode", event.target.value as AppSettings["experienceMode"])} value={settings.experienceMode}><option value="simple">Simple</option><option value="advanced">Advanced</option></select></label>
    ...
    <section className="settings-section"><div><p className="eyebrow">Recovery</p><h2>Local behavior</h2></div>
      ...
      <label className="toggle-setting"><span><strong>Show “What changes?” details</strong><small>Offer an optional advanced panel explaining what an operation preserves, rebuilds, or may remove.</small></span><input checked={settings.showPreservationWarnings} onChange={(event: { target: HTMLInputElement }) => update("showPreservationWarnings", event.target.checked)} type="checkbox" /></label>
    </section>
    <section className="settings-section"><div><p className="eyebrow">Updates and diagnostics</p><h2>Release behavior</h2></div>
