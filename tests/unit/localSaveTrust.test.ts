...
describe("local save trust", () => {
  it("never describes pending or failed writes as saved", () => {
    expect(localSaveStatusLabel(state("pending"), null, true)).toBe("Local changes waiting to save");
    expect(localSaveStatusLabel(state("saving"), null, true)).toBe("Saving local changes…");
  ...
  it("reports saved state only after a successful persistence revision", () => {
    expect(localSaveStatusLabel(state("saved"), null, true)).toBe("Local changes saved");
    expect(localSaveStatusLabel(state("saved"), "2 overlay objects · 4.2 KB", true)).toBe("2 overlay objects · 4.2 KB");
  });
