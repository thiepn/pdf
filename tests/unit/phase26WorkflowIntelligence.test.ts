...
describe("Phase 26 workflow intelligence",()=>{
  ...
  it("rejects non-terminal multi-output steps",()=>expect(()=>parseBatchRecipeJson(JSON.stringify({schemaVersion:3,name:"Bad",steps:[{id:"a",type:"split-fixed",pagesPerFile:2},{id:"b",type:"optimize"}],outputSuffix:"x"}))).toThrow(/final recipe step/));
});
